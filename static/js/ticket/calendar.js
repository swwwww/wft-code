...
var pickerEvent = {
    Init: function (elemid) {
        ...
        $('.pkg_circle_bottom ').on(M.click_tap,function(){
            pickerEvent.getNext();
        ...
        $('.pkg_circle_top').on(M.click_tap,function(){
            pickerEvent.getLast();
            var cur_val = $('body').find('.date_text').text();
            var date =cur_val.replace(/[\u4e00-\u9fa5]/g,'-');
            var date_str = date.split('-');
        ...
        for (var i = 0; i < tds.length; i++) {
            ...
            console.log($.trim(t_exchange(e)));
            console.log($.trim(t_exchange(s)));
...
var commonUtil = {
    ...
    chooseClick: function (sender) {
        ...
        if (el != null) {
            ...
            $('.ticket-price').find('mark').text(price);
            $('#order_id').val(order_id);
            $('.tips').find('.unsubscribe').remove();
            $('.tips').append('<a class="introduce unsubscribe">' +'<p>' + '<mark>退订说明：</mark>' + '<span>' + intro + '</span>' + '<i></i>' + '</p>' + '</a>')
            $('.num_ticket').find('span').remove();
            $('.num_ticket').find('p').append('<span>' + ' ，仅剩' + '<mark>'+ num + '</mark>' + '张' + '</span>');
            calculate();
            pickerEvent.remove();
            $('.calendar-matte').remove();
        }
...
var order_time = $('#order_time'),
    max_num_b = $('.num_ticket').attr('data-max'),
    order_start = order_time.attr('data-start'),
    order_end = order_time.attr('data-end'),
    id = GetQueryString('coupon_id'),
    tid = GetQueryString('order_id');
...
function AjaxTime(start_time,end_time){
    if((start_time >=order_start && start_time <= order_end) || (end_time >= order_start && end_time <= order_end) || (start_time <= order_start && end_time >= order_end)){
        ...
        dao.getAllCalendar(param, function (res) {
            if(res.status ==1){
                var data =res['data'];
                console.log(data);
